...
contract QuasarBondDepository is PolicyOwnable {
    ...
    function deposit( 
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( QUAS ).safeTransfer( DAO, fee ); 
        }
    ...
    function adjust() internal {
        ...
        if( adjustment.rate != 0 && block.timestamp >= timeCanAdjust ) {
            ...
            adjustment.lastTime = uint(block.timestamp);
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( QUAS ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( QUAS ).totalSupply();
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or QUAS) to the DAO
     *  @return bool
     */
